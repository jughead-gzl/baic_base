...
public:
    ...
    explicit RingBuffer() : buffer_(N + 1), read_idx_(0), write_idx_(0) {}
    ...
    {
        {
            ...
            // 等待直到缓冲区不满
            not_full_.wait(lck, [this]() { return !is_full(); });
    ...
    bool try_push_for(U && item, std::chrono::duration<Rep, Period> timeout)
    {
        std::unique_lock<std::mutex> lck(mtx_);
        // 等待直到缓冲区不满
        if (not_full_.wait_for(lck, timeout, [this]() { return !is_full(); }))
        {
            buffer_[write_idx_] = std::forward<U>(item);
            write_idx_ = (write_idx_ + 1) % buffer_.size();
    ...
    {
        std::unique_lock<std::mutex> lck(mtx_);
        not_empty_.wait(lck, [this]() { return !is_empty(); });
    ...
    {
        std::unique_lock<std::mutex> lck(mtx_);
        if (not_empty_.wait_for(lck, timeout, [this]() { return !is_empty(); }))
        {
            out = std::move(buffer_[read_idx_]);
            read_idx_ = (read_idx_ + 1) % buffer_.size();
            lck.unlock();
