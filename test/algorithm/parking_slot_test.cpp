...
void TestConstructorAndGetters()
{
    ...
    Slot slot(timestamp,
              42,
              structure::perception::ParkingSlotGeometryType::PARALLEL_2,
              structure::perception::ParkingSlotPhysicalType::COMMON_1,
              center,
              vertices);

    assert(slot.GetTimestamp().Nanoseconds() == TS::FromSeconds(10).Nanoseconds());
    assert(slot.GetID() == 42);
    assert(slot.GetGeometryType() == structure::perception::ParkingSlotGeometryType::PARALLEL_2);
    assert(slot.GetPhysicalType() == structure::perception::ParkingSlotPhysicalType::COMMON_1);
...
void TestSettersAndState()
{
    ...
    slot.SetID(7);
    slot.SetGeometryType(structure::perception::ParkingSlotGeometryType::PERPENDICULAR_1);
    slot.SetPhysicalType(structure::perception::ParkingSlotPhysicalType::MECHANICAL_3);
    slot.SetCenter(Center(3.0, 4.0, 0.0));
    slot.SetAvailable(true);

    assert(slot.GetTimestamp().Nanoseconds() == TS::FromMilliseconds(123).Nanoseconds());
    assert(slot.GetID() == 7);
    assert(slot.GetGeometryType() == structure::perception::ParkingSlotGeometryType::PERPENDICULAR_1);
    assert(slot.GetPhysicalType() == structure::perception::ParkingSlotPhysicalType::MECHANICAL_3);
    assert(std::abs(slot.GetCenter().GetX() - 3.0) < 1e-12);
    assert(std::abs(slot.GetCenter().GetY() - 4.0) < 1e-12);
